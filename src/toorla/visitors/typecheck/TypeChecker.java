...
public class TypeChecker extends Visitor<Void> {
    ...
    public Void visit(Return returnStat) {
        ...
        if(!TypeChecker.isFirstSubTypeOfSecond(actualReturnedType,formalReturnType,classHierarchy))
            returnStat.addError(new CompileErrorException(String.format("Expression returned by this method " + "must be %s", actualReturnedType.toString()), returnStat.line, returnStat.col));
        return null;
    }
    ...
    public Void visit(ClassDeclaration classDeclaration) {
        ...
        if(parentName != null) {
            if (!classHierarchy.doesGraphContainNode(parentName))
                classDeclaration.addError(new ClassNotDeclaredException(parentName, classDeclaration.getParentName().line, classDeclaration.getParentName().col));
            else
                if(classHierarchy.isSecondNodeAncestorOf(parentName, name)) {
                    classDeclaration.addError(new CycleFoundInInheritanceException(classDeclaration.getParentName().line, classDeclaration.col, name, parentName));
                }
        }
    ...
    public Void visit(EntryClassDeclaration entryClassDeclaration) {
        try {
            ...
            if(numOfEntryClasses >= 2) {
                entryClassDeclaration.addError(new MoreThanOneEntryClassException(entryClassDeclaration.getName().line,
            ...
            try {
                MethodSymbolTableItem mainMethod = (MethodSymbolTableItem) classItem.getSymbolTable().get(MethodSymbolTableItem.methodModifier + "main");
                if(!(mainMethod.getArgumentsTypes().isEmpty() && mainMethod.getReturnType() instanceof IntType)) {
                    if (numOfEntryClasses == 1)
                        throw new ItemNotFoundException();
                }
            }
            catch(ItemNotFoundException mainMethodNotFound) {
                entryClassDeclaration.addError(new MainMethodNotFoundException(entryClassDeclaration.getName().line, entryClassDeclaration.getName().col));
            }
        }
        catch(ItemNotFoundException ignored) {
        }
    ...
    public Void visit(ParameterDeclaration parameterDeclaration) {
        ...
        if(!classHierarchy.doesGraphContainNode(typeName) && parameterDeclaration.getType() instanceof UserDefinedType)
            parameterDeclaration.addError(new ClassNotDeclaredException(typeName, parameterDeclaration.line, parameterDeclaration.col));
        SymbolTable.define();
        return null;
    }
    ...
    public Void visit(FieldDeclaration fieldDeclaration) {
        ...
        if(!classHierarchy.doesGraphContainNode(typeName) && fieldDeclaration.getType() instanceof UserDefinedType) {
            fieldDeclaration.addError(new ClassNotDeclaredException(typeName, fieldDeclaration.line, fieldDeclaration.col));
        }
        return null;
    }
